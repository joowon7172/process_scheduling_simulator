...
class SPN:
    def __init__(self, process_input, CPU_input):
        ...
        self.CPU = CPU_input # SPN input CPU
        self.readyQueue = op.SPN_readyQueue() # spn readyqueue
            ...
            if self.readyQueue.peek():  # 대기 큐가 빈 상태가 아니라면     
                if self.CPU.CPU_running is False: # CPU가 작동중이지 않다면
                    ...
                    if self.ready_process.bt <= 0 and self.ready_process not in self.readyQueue.items:
                        self.terminated_state_spn(self.ready_process, start_time)                        # process finish
    ...
    def terminated_state_spn(self, ready_process, start_time): # 종료
